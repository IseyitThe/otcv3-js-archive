...
const menu_items = 
{
    script_items:
    {
        setup_menu_items: function()
        {
            ...
            menu_items.script_items.start_separator = menu.create_slider_int("           Semirage Assist", -1, 0);
            ...
            menu_items.script_items.indicators = menu.create_multi_dropdown("Display indicators", ["Autofire", "FOV", "Safety", "Bodyaim", "Resolver override", "Autowall", "AA", "Fakelag", "MM"]);
...
const utilities = 
{
    ...
    game: 
    {
        ...
        get_dynamic_fov: function(enemy_array, min_fov, max_fov)
        {
            ...
            const new_fov = closest_distance != Infinity ? math.clamp((5500 / closest_distance) * 0.8, min_fov, max_fov) : min_fov; //Could probably offer a choice between "simple" and "advanced" dynamic FOV handler modes, where advanced lets you change the base distance and the modifier thing
            return new_fov;
        }
    },
    ...
    log: function(text)
    {
        Cheat.PrintColor([199, 150, 255, 255], "[Hit Logs] ");
        Cheat.Print(text + "\n");
    },
    ...
    {
        this.log(text);
        Cheat.PrintChat(" \x02[Hit Logs] \x01" + text);
    }
};
    ...
    renderer:
    {
        helpers: 
        {
            ...
            render_indicators: function()
            {
                ...
                const mode_colors = [[255, 255, 255, 200], [196, 138, 2, 200], [0, 255, 0, 200], [255, 255, 255, 200]]; //generalized for ease of use

                if(indicator_selections & (1 << 0))
                {
                    render_indicator("MT", aim_data.active ? mode_colors[2] : mode_colors[0]);
                }

                if(indicator_selections & (1 << 1))
                {
                    render_indicator("FOV: " + aim_data.fov.toString(), mode_colors[3]);
                }
            ...
            render_mm_info: function()
            {
                ...
                for(var i = 0; i < players_array_length; i++)
                {
                    ...
                    if(Entity.IsValid(player))
                    {
                        ...
                        Render.StringCustom(base_x, base_y, 1, ((Entity.IsBot(player) ? "BOT " : "") + player_name.trim() + " | Rank: " + utilities.matchmaking_ranks[Entity.GetProp(player, "CCSPlayerResource", "m_iCompetitiveRanking")] + " | Wins: " + Entity.GetProp(player, "CCSPlayerResource", "m_iCompetitiveWins")), Entity.IsEnemy(player) ? [255, 255, 122, 255] : [231, 166, 255, 255], info_font);
                        base_y += 15;
                    }
                }
            }
