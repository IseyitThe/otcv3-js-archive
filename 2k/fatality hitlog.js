...
function onPlayerHurt() {
    ...
    if (attacker === Entity.GetLocalPlayer() && victim !== Entity.GetLocalPlayer()) {
        ...
        if (target != null) {
            const hitMessage = [
                [[255, 255, 255, 255], " [fatality] Hit "],
                [0, victimName.substring(0, 28)],
                [[255, 255, 255, 255], " [fatality] in the "],
                [0, target.hitgroup],
                [[255, 255, 255, 255], " [fatality] for "],
                [0, Event.GetInt("dmg_health").toString()],
                [[255, 255, 255, 255], " [fatality] damage ("],
                [0, Event.GetInt("health") + " [fatality] health remaining"],
                [[255, 255, 255, 255], ")"]
            ];
            activeLogs.push({
                    text: hitMessage,
