...
function getUIValue(key, type, doTransform) {
    ...
    if ( type == "default" ) raw = UI.GetValue( "Misc", "JAVASCRIPT", "Script items", key );
    if ( type == "text" ) {
        raw = UI.GetString( "Misc", "JAVASCRIPT", "Script items", key );
...
function setUIEnabled(key, value) {
    UI.SetEnabled( "Misc", "JAVASCRIPT", "Script items", key, value );
}
...
UI.AddLabel("")
UI.AddLabel("             Clan Tag Changer")
UI.AddLabel("_________________________________________")
UI.AddCheckbox(menu.enabled);
UI.AddDropdown(menu.tags, collect_keys( tags, true ) );
UI.AddTextbox(menu.text );
UI.AddDropdown(menu.styles, collect_keys( animations, false ) );
UI.AddDropdown(menu.killDickMode, killdickModes );
UI.AddTextbox(menu.killDickLength );
UI.AddSliderInt(menu.speed, 0, 100 );
UI.AddLabel("_________________________________________")
UI.AddLabel("")
