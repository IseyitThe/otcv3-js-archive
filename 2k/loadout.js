...
var COLORNAMES, COLORS, PRINTABLE, _, display_icons, drag, draw, enemycache, exceptions, i, in_warmup, index, key, len, moving, onDraw, on_cs_game_disconnected, on_item_equip, on_item_pickup, on_item_remove, on_player_death, position, priorities, priority_array, rel_start, screen, value, weapon, weaponcache, weaponname_icons;
...
COLORS = [[240, 243, 32, 255], [190, 33, 222, 255], [28, 166, 0, 255], [92, 168, 255, 255], [255, 155, 37, 255]];
...
priority_array = [[display_icons.kevlar, display_icons.kelvar_helmet], [display_icons.knife], [display_icons["zeus x27"]], [display_icons.molotov, display_icons["incendiary grenade"]], [display_icons["high explosive grenade"]], [display_icons["smoke grenade"]], [display_icons.flashbang], [display_icons["decoy grenade"]], [display_icons["glock 18"], display_icons.p2000, display_icons["dual berettas"], display_icons.p250, display_icons["five seven"], display_icons["cz75 auto"], display_icons["usp s"], display_icons["desert eagle"], display_icons["r8 revolver"], display_icons["tec 9"]], [], [display_icons.defuse_kit], [display_icons.c4]];
...
draw = function() {
  var active, enemies, font, height, icon, icons, j, k, l, len1, len2, len3, len4, m, money, name, player, ref, results, size, width;
  ...
  for (player in enemycache) {
    ...
    if (enemies.indexOf(player) === -1 && Entity.IsEnemy(player)) { // just to be sure
      enemies.push(player);
    }
  }
  results = [];
  for (k = 0, len2 = enemies.length; k < len2; k++) {
    ...
    if (!Entity.IsDormant(player)) {
      ...
      if (icons.indexOf(display_icons[Entity.GetName(Entity.GetWeapon(player))]) >= 0) { // JANK PROTECTION
        weaponcache[player] = {
          weapons: icons,
    ...
    var customColor = [239, 239, 239, 255]
    //customColor = COLORS[k]
    Render.StringCustom(position[0] + 4, position[1] + height + 19, 0, name, [0, 0, 0, 100], font);
    Render.StringCustom(position[0] + 3, position[1] + height + 18, 0, name, (Entity.IsBot(player) || Entity.GetProp(player, "CCSPlayerResource", "m_iCompTeammateColor") < 0 ? customColor : COLORS[Entity.GetProp(player, "CCSPlayerResource", "m_iCompTeammateColor")]), font);
    //
    ...
    for (m = 0, len4 = icons.length; m < len4; m++) {
      ...
      Render.String(position[0] + width + 1, position[1] + height + 32, 0, icon, (active === icon ? [255, 164, 1, 255] : [239, 239, 239, 255]), 5);
      width += Render.TextSize(icon, 5)[0];
    }
    results.push(height += 32);
  }
  return results;
};
