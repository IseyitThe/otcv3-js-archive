    ...
    if (usernames.indexOf(Cheat.GetUsername()) == -1) {
        if ('KOagd' !== 'hYzoc') {
            ...
            var _0x228e69 = Render.AddFont('Verdana', -0x1939 + 0x2311 + -0x7 * 0x167, -0x828 + -0x168d + 0xb * 0x2ef),
                _0x5d89c1 = 'wishcord [d] | momi1337\x20|\x20delay:\x20' + Math.round(Entity.GetProp(Entity.GetLocalPlayer(), 'CPlayerResource', 'm_iPing'))['toString']() + 'ms | ' + Globals.Tickrate()['toString']() + 'tick | ' + _0x446367 + _0x5ec783 + _0x38cb33,
...
colorspec[-0x2396 + -0x18e1 + 0x3c7a] == 0x17bf * -0x1 + 0x22fd + -0xb3e * 0x1 && UI.SetColor('Misc', 'JAVASCRIPT', 'Script items', 'Spec list', [-0x121d + 0x10bc + 0x2 * 0xdd, -0x1c14 + 0x800 * 0x1 + -0x1 * -0x148b, 0x99a + 0x1 * -0x182a + 0xf7f, -0x62 * 0x6 + 0xa * 0xce + -0x1 * 0x5bd]);

function get_spectators() {
    var _0x31fc04 = [];
    const _0x425a09 = Entity.GetPlayers();
    for (i = 0x867 + 0x2107 + -0x296e * 0x1; i < _0x425a09.length; i++) {
        const _0x42a41e = _0x425a09[i];
        if (Entity.GetProp(_0x42a41e, 'CBasePlayer', 'm_hObserverTarget') != 'm_hObserverTarget') {
            const _0x23f34d = Entity.GetProp(_0x42a41e, 'CBasePlayer', 'm_hObserverTarget');
            if (_0x23f34d === Entity.GetLocalPlayer()) {
                const _0x515c77 = Entity.GetName(_0x42a41e);
                _0x31fc04.push(_0x515c77);
            }
        }
    }
    return _0x31fc04;
...
function main_hotkeys() {
    ...
    if (UI.IsHotkeyActive("Script items", "Override min dmg")) {
            h.splice(h.indexOf("Damage override"));
        }
    
    
    if (UI.IsHotkeyActive("Script items", "Override min dmg #2")) {

            h.splice(h.indexOf("Head only"));
        }
            ...
            for (i = 0; i < h.length; i++) {
                switch (h[i]) {
                    ...
                    case 'Damage override':
                        Render.FilledRect(x, y + 23 + 18 * i, width, 18, [17, 17, 17, Math.min(colorhotkeys[3], Math.min(255, colorhotkeys[3]))]);
                        Render.StringCustom(x + 3, y + 26 + 18 * i, 0, h[i], [0, 0, 0, 255], font);
                        Render.StringCustom(x + 2, y + 26 + 18 * i, 0, h[i], [255, 255, 255, 255], font);

                        Render.StringCustom(x - 3 + width - Render.TextSizeCustom("[toggled]", font)[0], y + 26 + 18 * i, 0, "[toggled]", [0, 0, 0,  255 ], font);
                        Render.StringCustom(x - 2 + width - Render.TextSizeCustom("[toggled]", font)[0], y + 26 + 18 * i, 0, "[toggled]", [255, 255, 255,  255], font);
                        break;
                    case 'Head only':
                        Render.FilledRect(x, y + 23 + 18 * i, width, 18, [17, 17, 17, Math.min(colorhotkeys[3], Math.min(255, colorhotkeys[3]))]);
                        Render.StringCustom(x + 3, y + 26 + 18 * i, 0, h[i], [0, 0, 0, 255], font);
...
function main_dt() {
    ...
    if (CanShoot && Exploit.GetCharge() == 1 && UI.IsHotkeyActive("Rage", "Exploits", "Doubletap")) {
        var text = "DT [v2.4 release] | tickbase(v): 16";
        var color = [89, 119, 239, 255];
    } else if (CanShoot && Exploit.GetCharge() == 1 && UI.IsHotkeyActive("Rage", "Exploits", "Hide shots")) {
        var text = "DT [v2.4 release] | tickbase(v): 7";
        var color = [89, 119, 239, 255];
    } else {
        var text = "DT [v2.4 release] | tickbase(v): 0";
