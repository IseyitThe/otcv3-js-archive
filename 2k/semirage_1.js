...
function update_settings()
{
    ...
    script_config.legitaa_juke_active = UI.IsHotkeyActive(js_items, "Legit AA juke (only in rage)");
...
{
    return [190 - (abs_yaw * 85 / 40), 40 + (abs_yaw * 146 / 60), 10, desired_alpha];
}

function render_outlined_indicator(x, y, text, color)
{
    Render.String(x - 1, y - 1, 1, text, [0, 0, 0, 255], 4);
    Render.String(x, y, 1, text, color, 4);
}
...
function handle_indicators()
{
    if(script_config.script_active && script_config.indicator_picks)
    {
        ...
        var screen_center_x = 65; //screensize[0] * 1;
        if(Entity.IsValid(local))
        {
            ...
            if(Entity.IsAlive(local))
            {
                if(script_config.indicator_picks & (1 << 0))
                {
                    ...
                    if(converted_ragebot_type != -1)
                    {
                        ...
                        var string = "FOV: " + weapon_cur_fov; //" FOV: " + weapon_cur_fov;
                        text += string;
                    }
                    render_outlined_indicator(screen_center_x, base_yaw, text, (is_aimbot_active ? [77.5, 186, 10, 200] : [77.5, 186, 10, 200]));
                    base_yaw += 20;
                ...
                {
                    var color = script_config.autowall_active || script_config.autowall_mode == 2 ? [77.5, 186, 10, 200] : (script_config.autowall_mode == 0 ? [255, 25, 30, 200] : [255, 25, 30, 200]);
                ...
                {
                    var text = (script_config.rbot_active ? "TM" : "");
                    var col = script_config.rbot_active ? [77.5, 186, 10, 200] : [255, 25, 30, 200];
            ...
            if(script_config.indicator_picks & (1 << 7) && Input.IsKeyPressed(0x09)) //Tab
            {
                ...
                Render.String(base_x, base_y, 1, "MM Data", [255, 255, 255, 255], 4);
                base_y += 15;
                ...
                if(player_arr_length > 0)
                {
                    for(var i = 0; i < player_arr_length; i++)
                    {
                        if(Entity.IsValid(players[i]))
                        {
                            ...
                            Render.String(base_x, base_y, 1, final_string, [255, 255, 255, 255], 4)
                            base_y += 15;
                        }
                    }
                }
        ...
        if(script_config.indicator_picks & (1 << 6)) //gay watermark
        {
            ...
            var watermark_string = "imaginebeinga.live | $uicidE";
            if(are_we_ingame)
            {
            ...
            var string_size = Render.TextSize(watermark_string);
            Render.GradientRect(screensize[0] * 0.99 - string_size[0], 8, string_size[0] + 10, 20, 1, [0, 0, 0, 150], [0, 0, 0, 100]);
            Render.String(screensize[0] * 0.99 - string_size[0] + 5, 11, 0, watermark_string, accent_color);
