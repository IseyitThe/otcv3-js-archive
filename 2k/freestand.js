...
localplayer = {    
    ...
    health() {
        return Entity.GetProp(localplayer.local(), "CBasePlayer", "m_iHealth");
    },
    moving() {
...
entity = {
    ...
    getClosestEnemy() {
        ...
        for (i = 0; i < 64; i++) {
            ...
            if (d < dist && Entity.IsAlive(enms[i])) {dist = d; enm = enms[i];} 
        }
        return [enm, dist];
    },
    getCrosshairEnemy() {
...
trace = {
    ...
    getHeadPos(entity_id, entity_angles) {
        var entity_vec = trace.angle_to_vec( entity_angles[0], entity_angles[1] );
        var entity_pos = Entity.GetRenderOrigin( entity_id );
        entity_pos[2] += 60;
    ...
    getViewpoint(entity_id, entity_angles) {
        var entity_vec = trace.angle_to_vec( entity_angles[0], entity_angles[1] );
        var entity_pos = Entity.GetRenderOrigin( entity_id );
        entity_pos[2] += 60;
        var stop = [ entity_pos[ 0 ] + entity_vec[0] * 512, entity_pos[1] + entity_vec[1] * 512, ( entity_pos[2] )  + entity_vec[2] * 512 ];
        return stop;
    }
}
AA = {
    ...
    antiaims: {
        ...
        edgeDetect(std) {
            if (std) {
                ...
                if (dist_left < 50 || dist_right < 50 || left[2] < 50 || right[2] < 50) {
                    hposLeft = trace.getHeadPos(local, [0, angle[1] + 90]);
                    hposRight = trace.getHeadPos(local, [0, angle[1] - 90]);
                    pos = trace.getViewpoint(local, [0, angle[1]]);
                    resLeft = Trace.Line(local, hposLeft, pos);
                    resRight = Trace.Line(local, hposRight, pos);
        ...
        freestand() {
            ...
            if (target != null && !Entity.IsDormant(target) && Entity.IsAlive(target) && Entity.IsValid(target) && Entity.IsAlive(local)) {
                ...
                res_line = Trace.Line(target, pos, eye)
                res_bullet = Trace.Bullet(local, target, eye, pos);
                if ((Entity.IsValid(res_bullet[0]) && Entity.IsAlive(res_bullet[0])) || res_line[1] > .05 || res_predict[1] > .5) {
                    hasTarget = true;
...
function DRAW() {
    ...
    UI.SetEnabled("Target indicator", (aa == 5 && e));
    if (UI.GetValue("Target indicator") && aa == 5 && e && target != null) {
        t = "Target: " + hasTarget ? Entity.GetName(target) : "none";
        c = hasTarget ? [15,250,35,255] : [255,15,35,255];
        p = Render.GetScreenSize();
        Render.String(p[0] / 2 - Render.TextSize(t)[0] / 2, p[1] / 2 + (p[1] / 3), 0, t, c);
    }
}
...
UI.AddCheckbox("Target indicator");
UI.AddHotkey("Toggle");
UI.AddHotkey("Left");
UI.AddHotkey("Right");
...
UI.SetEnabled("Target indicator", 0);
