...
function showActiveItems(rage) {
    ...
    if (getValue("Show Indicators")) {
		...
		setVisibility("NOT AVALIABLE IN FREE", true);
...
function showSemiRageItems() {
	...
	setVisibility("NOT AVALIABLE IN FREE", false);
...
function initializeMenuItems() {
    ...
    UI.AddLabel("Please consider buying my");
    UI.AddLabel("paid script. Thank you.");
    UI.AddLabel("Version: 1.1 FREE");
    ...
    UI.AddDropdown("Yaw Mode", ["Random"]);
    UI.AddDropdown("Jitter Mode", ["Static", "Random"]);
	UI.AddDropdown("Freestanding Mode", ["NOT","AVALIABLE","IN THE","FREE","VERSION"]);
    UI.AddMultiDropdown("Invert On", ["Shot", "Slow Walk", "Hurt", "Enemy Fire", "Luck"]);
    UI.AddMultiDropdown("Disable On", ["NOT","AVALIABLE","IN THE","FREE","VERSION"]);
    ...
    UI.AddDropdown("Fake Lag", ["Disabled", "HVH", "Match-Making", "Maximum", "Velocity"]);
    UI.AddHotkey("Maximum Fake Lag");
    UI.AddSliderInt("0x07", 0, 0);
    UI.AddDropdown("Clan Tag", ["You", "are", "forced", "to", "enable", "my", "clantag", " in the free version"]);
	...
	UI.AddDropdown("Auto Wall Mode", ["Rage"]);
	...
	UI.AddCheckbox("NOT AVALIABLE IN FREE");
	UI.AddCheckbox("Show Death Mark");
	UI.AddCheckbox("Show Bullet Tracer");
	UI.AddSliderInt("Bullet Tracer Thickness", 2, 50);
    UI.AddMultiDropdown("Show Keybind States", ["Inverter", "PLACEHOLDER - NOT AVALIABLE IN FREE", "Damage Modes", "Accuracy Modes", "Maximum FOV On Key", "Maximum Fake Lag", " Slow Walk", "Auto Wall", "Fake Duck"]);
    UI.AddDropdown("Desync Indicator", ["Disabled", "Equilateral Triangle"]);
    UI.AddDropdown("Theme", ["Blanco", "Negro"]);
	UI.AddDropdown("Font", ["Bold"]);
...
function setRageAA(invert) {
    ...
    if (invert) {
        basedir = getRandomInteger( - 10, 0);
        AntiAim.SetFakeOffset(0);
        AntiAim.SetRealOffset( - 50 - basedir);
        AntiAim.SetLBYOffset(getRandomInteger(getValue("Min LBY"), getValue("Max LBY")));
    } else {
        basedir = getRandomInteger(0, 10);
        AntiAim.SetFakeOffset(0);
        AntiAim.SetRealOffset(50 + basedir);
        AntiAim.SetLBYOffset( - getRandomInteger(getValue("Min LBY"), getValue("Max LBY")));
    }
	
    if (getValue("Yaw Mode")==0) {
...
function weaponDamageUpdate() {
    ...
    if (Entity.IsValid(rageTarget) == true && Entity.IsAlive(rageTarget) && Entity.IsDormant(rageTarget) == false) {
        ...
        if (getValue("Automatic Mindmg")) {
            if (Entity.GetProp(rageTarget, "CBasePlayer", "m_iHealth") < UI.GetValue("Rage", getWeapon(Entity.GetName(Entity.GetWeapon(Entity.GetLocalPlayer()))).toUpperCase(), "Targeting", "Minimum damage") && Entity.GetProp(rageTarget, "CBasePlayer", "m_iHealth") > 0 && UI.GetValue("Rage", getWeapon(Entity.GetName(Entity.GetWeapon(Entity.GetLocalPlayer()))).toUpperCase(), "Targeting", "Minimum damage")<101) {
...
function getWeapon(wep) {
    if (wep == "awp") {
        ...
        return "Autosniper";
    } else if (typeof(wep) == "string" && (wep == "desert eagle" || wep.indexOf(" Kp") != -1 || wep.indexOf("r8") != -1)) {
        return "Heavy Pistol";
    } else if (typeof(wep) == "string" && (wep == "elite" || wep == "fiveseven" || wep == "p2000" || wep == "glock" || wep == "tec9" || wep == "dual berettas" || wep.indexOf("  ^T") != -1 || wep.indexOf("usp") != -1)) {
        return 'Pistol';
    } else {
        return 'General';
    }
...
function fastDuckUpdate() {
	...
	//Entity.SetProp(Entity.GetLocalPlayer(), "CPointCamera", "m_bActive",true)
	//Cheat.PrintChat(Entity.GetProp(Entity.GetLocalPlayer(), "CCSPlayer", "m_vecOrigin")+"\n");
	//UI.SetEnabled( "Anti-Aim", "Extra", "Fake duck", true );
	if(valve){
...
var x5 = Math.ceil(screen_size[0] * 0.01);
var y5 = Math.ceil(screen_size[1] * 0.60);
...
function drawHitAndMiss() {
    ...
    Render.String(x3 + 15, y3 + 35, 0, "Weapon: " + Entity.GetName(Entity.GetWeapon(Entity.GetLocalPlayer())) + " shot: " + shot[Entity.GetName(Entity.GetWeapon(Entity.GetLocalPlayer()))] + " hit: " + hit[Entity.GetName(Entity.GetWeapon(Entity.GetLocalPlayer()))], theme_text, font_size);
    Render.String(x3 + 15, y3 + 55, 0, "Current " + (percentage * 100).toFixed(0) + "%", theme_text, font_size);
    Render.Rect(x3 + 100, y3 + 60, 145, 6, theme_bar_border);
    drawFilledOrRainbowRect(x3 + 100, y3 + 61, Math.floor(145 * percentage), 4);
    Render.String(x3 + 15, y3 + 75, 0, "Total " + (percentageSum * 100).toFixed(0) + "%", theme_text, font_size);
...
function drawStatusIndicator() {
    ...
    Render.String(x2 + 60, y2 + 35, 0, "Current Target: " + targetName, theme_text, font_size);
    Render.String(x2 + 60, y2 + 55, 0, "Head Scale ", theme_text, font_size);
    Render.Rect(x2 + 135, y2 + 59, 155, 6, theme_bar_border);
    drawFilledOrRainbowRect(x2 + 135, y2 + 60, Math.floor(155 * (headScale / 100)), 4);
    Render.String(x2 + 60, y2 + 75, 0, "Body Scale ", theme_text, font_size);
    Render.Rect(x2 + 135, y2 + 79, 155, 6, theme_bar_border);
    drawFilledOrRainbowRect(x2 + 135, y2 + 80, Math.floor(155 * (bodyScale / 100)), 4);
    Render.String(x2 + 60, y2 + 95, 0, "Damage Mode: " + damageMode, theme_text, font_size);
    Render.String(x2 + 60, y2 + 115, 0, "Expected Damage: " + expectedDamage + "HP", theme_text, font_size);
    Render.String(x2 + 60, y2 + 135, 0, "Dodge Bruteforce: " + dodgeBruteforce, theme_text, font_size);
    Render.String(x2 + 60, y2 + 155, 0, "Desync ", theme_text, font_size);
    Render.Rect(x2 + 135, y2 + 159, 155, 6, theme_bar_border);
    drawFilledOrRainbowRect(x2 + 135, y2 + 160, Math.floor(155 * (desyncSize / 120)), 4);
    Render.String(x2 + 60, y2 + 175, 0, "Fake Lag ", theme_text, font_size);
