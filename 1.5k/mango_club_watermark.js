...
const __hotkey_list = UI.AddCheckbox(["Visuals", "SUBTAB_MGR", "Watermark", "SHEET_MGR", "Watermark"], "Display active hotkeys");
const __watermark = UI.AddCheckbox(["Visuals", "SUBTAB_MGR", "Watermark", "SHEET_MGR", "Watermark"], "Display watermark");

const hotkey_list_accent = UI.AddColorPicker(["Visuals", "SUBTAB_MGR", "Watermark", "SHEET_MGR", "Watermark"], "Hotkey list accent color");
const watermark_accent = UI.AddColorPicker(["Visuals", "SUBTAB_MGR", "Watermark", "SHEET_MGR", "Watermark"], "Watermark accent color");
...
const draggable = {
    ...
    create_draggable: function(starting_size_x, starting_size_y, callback) {
        ...
        const slider_x = UI.AddSliderInt(["Visuals", "SUBTAB_MGR", "Watermark", "SHEET_MGR", "Watermark"], "_draggable_" + this.draggables.length + "_x", 0, screen_size[0]);
        const slider_y = UI.AddSliderInt(["Visuals", "SUBTAB_MGR", "Watermark", "SHEET_MGR", "Watermark"], "_draggable_" + this.draggables.length + "_y", 0, screen_size[1]);
...
const watermark = function() {
    if(UI.GetValue(__watermark)) {
        var watermark_str = " mango.club | " + Cheat.GetUsername() + " | ";
        ...
        const watermark_font = Render.AddFont("Georgia.ttf", 11, 600);
        ...
        Render.FilledRect(render_position[0] - 1, render_position[1], string_size[0], string_size[1] + 3, [10, 10, 10, 165]);
        Render.String(render_position[0] + 3, render_position[1] + 2, 0, watermark_str, [0, 0, 0, 195], watermark_font);
        Render.String(render_position[0] + 3, render_position[1] + 1, 0, watermark_str, [191, 255, 0, 255], watermark_font);
    }
};
...
draggable.create_draggable(195, 20, function(menu_open) {
    if(UI.GetValue(__hotkey_list)) {
        ...
        if(hotkey_list.list_internal_data.alpha > 0) {
            ...
            const hotkey_title_font = Render.AddFont("Georgia.ttf", 11, 650);

            const render_position = [this.pos[0], this.pos[1]];
            Render.FilledRect(render_position[0], render_position[1], this.size[0], 2, [hotkey_accent_color[0], hotkey_accent_color[1], hotkey_accent_color[2], 225 * hotkey_list.list_internal_data.alpha]);
            Render.FilledRect(render_position[0], render_position[1] + 2, this.size[0], this.size[1] - 1, [10, 10, 10, 165 * hotkey_list.list_internal_data.alpha]);
            Render.String(render_position[0] + this.size[0] / 2, render_position[1] + this.size[1] / 5 + 1, 1, "MangoKey List", [0, 0, 0, 225 * hotkey_list.list_internal_data.alpha], hotkey_title_font);
            Render.String(render_position[0] + this.size[0] / 2, render_position[1] + this.size[1] / 5, 1, "MangoKey List", [191, 255, 0, 255 * hotkey_list.list_internal_data.alpha], hotkey_title_font);
            render_position[1] += this.size[1];

            if(!menu_open) {
                const hotkey_font = Render.AddFont("Georgia.ttf", 11, 400);
                for(var i in hotkey_list) {
                    if(hotkey_list[i] && hotkey_list[i].name) {
                        ...
                        if(hotkey_list[i].alpha > 0)
                        {
                            ...
                            Render.String(render_position[0] + 2, render_position[1] + 1, 0, i, [0, 0, 0, hotkey_list[i].alpha * 225], hotkey_font)
                            Render.String(render_position[0] + 2, render_position[1], 0, i, [255, 255, 255, hotkey_list[i].alpha * 225], hotkey_font);

                            Render.String(render_position[0] + this.size[0] - 1 - measured_str_mode[0], render_position[1] + 1, 0, mode, [0, 0, 0, hotkey_list[i].alpha * 225], hotkey_font);
                            Render.String(render_position[0] + this.size[0] - 1 - measured_str_mode[0], render_position[1], 0, mode, [255, 255, 255, hotkey_list[i].alpha * 225], hotkey_font);
