UI.AddLabel('               ');
UI.AddLabel('             "Nades" script');
UI.AddCheckbox("Touch me for setup")
UI.AddColorPicker("Nade color")
UI.AddCheckbox("Auto-Throw")
UI.AddHotkey("Auto-Throw key")
UI.AddCheckbox("Render on all map")
...
function atv(pitch, yaw) {
  ...
  function menu_1() {
    var enabled = UI.GetValue(scriptitems, "Touch me for setup");
	UI.SetEnabled(scriptitems, "Nade color", enabled);
	UI.SetEnabled(scriptitems, "Auto-Throw", enabled);
    UI.SetEnabled(scriptitems, "Render on all map", enabled);
	var enabled = UI.GetValue(scriptitems, "Auto-Throw");
	UI.SetEnabled(scriptitems, "Auto-Throw key", enabled);
  }
...
function draw() {
  ...
  color  = UI.GetColor("Misc", "JAVASCRIPT", "Script items", "Nade color")
  ...
  if (enabled)
  {
    ...
    // if(UI.IsHotkeyActive("Misc", "JAVASCRIPT", "Script items", "Auto-Throw key") && UI.GetValue("Misc", "JAVASCRIPT", "Script items", "Auto-Throw"))
    // {
    //   get_velocity()
    // }
    for(var i = 0; i < coordslenght; i++) {
      ...
      if(weaponnames[crd[2]].indexOf(weapon) >= 0) {
        ...
        var nadeallmap = UI.GetValue(scriptitems,"Render on all map");
    ...
    for(var i = 0; i < coordslenght; i++) {
      ...
      if(weaponnames[crd[2]].indexOf(weapon) >= 0) {
        ...
        if(dis([crd[3], crd[4], crd[5]], Entity.GetRenderOrigin(Entity.GetLocalPlayer())) < 10) {
          ...
          if(UI.IsHotkeyActive("Misc", "JAVASCRIPT", "Script items", "Auto-Throw key") && UI.GetValue("Misc", "JAVASCRIPT", "Script items", "Auto-Throw") && !running)
          {
