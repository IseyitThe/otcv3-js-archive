...
function do_indicators( )
{
    ...
    render_string(10, y - 449, 0, "FAKE", [10, 10, 10, 125], 4);
    render_string(10, y - 450, 0, "FAKE", [192 - (abs * 71 / 60), 32 + (abs * 146 / 60), 28, 200], 4);

    // Render the bar's background
    render_filled_rect(12, y - 418, 64, 4, [10, 10, 10, 125]);

    // Draw this small tile to fix a small issue that was driving me crazy.
    render_filled_rect(43, y - 417, 1, 2, [232, 232, 232, 200]);

    // Render the desync's length under the bar.
    render_string(41, y - 413, 1, abs.toString( ), [232, 232, 232, 200], 3);
    render_circle(48, y - 411, 1, [232, 232, 232, 200]);
    ...
    if (delta > 0)
    {
        ...
        render_filled_rect(44, y - 417, abs * 31 / 60, 2, [232, 232, 232, 200]);
        return;
    }

    // If our delta is not positive, than our desync is headed to the left.
    // So, fill the bar from the center to the left.
    render_filled_rect(44 - abs * 31 / 60, y - 417, abs * 31 / 60, 2, [232, 232, 232, 200]);
}
